...
class BroadcastHandler:
    ...
    async def message_copy_wrapper(
        ...
        Returns:
            Union[Message, List[Message]]: The copied message(s).
        """

        async def copy_and_pin() -> Union[Message, List[Message]]:
        ...
        try:
            ...
            await asyncio.sleep(float(e.x))
            return await copy_and_pin()

    @staticmethod
    async def cleanup_users(unsuccessful_ids: List[int], unsuccessful_ids_codex: List[int]) -> None:
        """Cleans up users from database based on their IDs.

        Parameters:
            unsuccessful_ids (List[int]): List of user unsuccessful id from teleshare to be delete from the database.
            unsuccessful_ids_codex (List[int]): List of user unsuccessful id from CodeXbotz to be delete from the database.
        """
        if unsuccessful_ids:
    ...
    async def broadcast_sender(cls, client: Client, message: Message, broadcast_config: BroadcastConfig) -> dict:
        ...
        Parameters:
            ...
            broadcast_config (BroadcastConfig): Configuration for broadcasting.
        ...
        for user_id in list(set(broadcast_config.user_ids + broadcast_config.user_ids_codex)):
            try:
                ...
                if user_id in broadcast_config.user_ids:
                    unsuccessful_ids.append(user_id)
                else:
                    unsuccessful_ids_codex.append(user_id)
